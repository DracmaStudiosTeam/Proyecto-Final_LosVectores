...
void opc2_0(char opcSel[][100], int opc)
{
	int i, j = opc, tecla, def = 1, selec = 1, var = 0; // Controladores
	do
	{
		for (i = 0; i <=strlen(opcSel[j]); i++)
		{
			opcSel[j][i] = toupper(opcSel[j][i]); // Pasar a mayusculas al titulo seleccionado
		}
		system("CLS"); // Limpiar la pantalla
		system("COLOR 0A"); //Color para la consola
		...
		gotoxy(3, 1); printf(opcSel[j]); // Imprimir el titulo del apartado escogido
